...
**LeetSolv** 是一个命令行复习工具，它利用自定义的 [SuperMemo 2](https://en.wikipedia.org/wiki/SuperMemo) 算法。该算法结合了 **熟悉度**、**重要性**、**推理能力** 和其他因素的变量，旨在通过实践来促进对算法的掌握——**而不是死记硬背**。
...
- [LeetSolv](#leetsolv)
  ...
  - [支持](#支持)
    - [常见问题解答](#常见问题解答)
      - [问：我应该一次性添加所有以前解决的 LeetCode 问题吗？](#问我应该一次性添加所有以前解决的-leetcode-问题吗)
      - [问：如果我想添加几个月前解决的一个旧问题怎么办？](#问如果我想添加几个月前解决的一个旧问题怎么办)
      - [问：使用一段时间后，我积累了太多到期的问题。](#问使用一段时间后我积累了太多到期的问题)
    - [文档](#文档-1)
    - [链接](#链接)
...
#### 问：我应该一次性添加所有以前解决的 LeetCode 问题吗？

答：**不应该。** LeetSolv 的调度算法依赖于您添加问题以计算未来复习的日期。在同一天添加所有问题会使时间表不准确，并且可能会在几天后导致大量无法管理的复习。

#### 问：如果我想添加几个月前解决的一个旧问题怎么办？

答：最好的方法是先重新解决该问题。成功再次解决后，使用 `leetsolv add` 在当天将其添加到 LeetSolv。这可确保系统具有准确的“上次查看”日期以供使用。

#### 问：使用一段时间后，我积累了太多到期的问题。

答：这是 **SM-2 算法** 的本质——如果你跳过几天或一次添加许多问题，到期列表可能会迅速增长。
为了使其易于管理，LeetSolv 引入了 **[到期优先级评分](#due-priority-scoring)**，该评分按重要性、熟悉度、逾期天数、复习次数和容易程度对到期问题进行排名。
不要一次性清除所有内容，只需首先关注**最高优先级的问题**。其余的可以安全地等到以后。

👉 提示：只添加你真正想复习的问题。LeetSolv 在你进行选择性添加时效果最佳，而不是批量添加所有内容。
...
**LeetSolv** - 一个 CLI 复习工具，它利用自定义的 SM-2 算法，旨在通过实践来掌握算法——而不是死记硬背。
