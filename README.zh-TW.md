...
當新增問題時，LeetSolv 會應用具有自定義因素（**熟悉度**、**重要性**和**推理**）的 SM-2 演算法，以計算下一次複習日期。此設計強化了**通過推理而不是死記硬背**來掌握資料結構和演算法的目標。
...
問題的**重要性**決定了基礎複習間隔，該間隔會隨著**熟悉度**和**推理**因子而呈指數增長。這種設計對於學習時間有限的學習者特別有用。例如：

- 初學者可以優先考慮知識建構列表，例如 NeetCode Blind 75 或 NeetCode 150，以建立堅實的基礎，而 NeetCode 250 可用於額外的練習。
- 準備特定公司面試的個人可以優先考慮公司特定的列表，以加強他們的面試準備。
...
- **原子寫入**：所有更新都使用帶有原子替換的臨時檔案，以**保證一致性**並**防止資料丟失**。
