...
**LeetSolv** 是一個命令列工具，專為 **資料結構與演算法 (DSA)** 問題複習而設計。它由一個客製化的 [SuperMemo 2](https://en.wikipedia.org/wiki/SuperMemo) 演算法驅動，該演算法結合了 **熟悉度**、**重要性** 和 **推理** 等變數。這種獨特的方法超越了死記硬背，透過 **刻意練習** 幫助您 **掌握** 複雜的演算法。
...
為了使其易於管理，LeetSolv 引入了 **[到期優先級評分](#due-priority-scoring)**，它按重要性、熟悉度、逾期天數、複習次數和容易程度對到期問題進行排名。
