...
當新增問題時，LeetSolv 會應用自定義因子：**熟悉度** (5 個等級)、**重要性** (4 個等級) 和 **推理** (3 個等級) 的 SM-2 演算法，以計算下一次複習日期。並採用**隨機化**因子避免了將複習集中到同一天。此設計強化了**通過推理而不是死記硬背**來掌握資料結構和演算法的目標。
...
您的**複習間隔**會根據問題的**重要性**自動設定。然後，這個間隔會隨著時間擴大，根據您對問題的**熟悉度**和**推理**程度而變化。
